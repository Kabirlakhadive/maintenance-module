...
import { Row, Col, Card, Typography, Space, Statistic, Progress, Alert, Tag, Divider, Table } from 'antd';
...
export const HardwareTrends: React.FC = () => {
  ...
  const getHealthStatusData = () => {
    ...
    return [
      { name: 'Healthy', value: statusCount.healthy, itemStyle: { color: '#52c41a' } },
      { name: 'Warning', value: statusCount.warning, itemStyle: { color: '#faad14' } },
      { name: 'Critical', value: statusCount.critical, itemStyle: { color: '#ff4d4f' } }
    ];
  };
  ...
  const cpuTrendOption = {
    ...
    tooltip: {
      ...
      formatter: (params: any) => {
        const data = params[0];
        return `CPU: ${data.value[1]}%<br/>${new Date(data.value[0]).toLocaleString()}`;
      }
    },
    xAxis: {
      type: 'time',
  ...
  const serverComparisonOption = {
    ...
    series: [
      ...
      },
      {
        name: 'Temperature',
        type: 'bar',
        data: prepareServerComparisonData('temperature').map(d => d.value / 100 * 100), // Normalize to percentage
    ...
    <Row gutter={[16, 16]}>
            ...
            <Col span={24}>
              <Space direction="vertical" style={{ width: '100%' }}>
                ...
                <div style={{ textAlign: 'center' }}>
                  <Statistic
                    title="Avg CPU Load"
                    value={servers.length > 0 ? Math.round(servers.reduce((sum, s) => sum + s.hardware.cpu.utilization_percent, 0) / servers.length) : 0}
                    suffix="%"
      ...
      <Col xs={24} lg={16}>
        <GrafanaPanel
          ...
          subtitle="Real-time temperature readings across infrastructure"
          height={350}
        >
          <Row gutter={[16, 16]}>
            <Col span={24}>
            ...
            <Col span={24}>
              <Row gutter={[16, 8]}>
                {servers
                  .filter(s => s.hardware.cpu.temperature_celsius.package > 70)
                  .slice(0, 3)
                  .map(server => (
                    <Col xs={24} sm={8} key={server.hostname}>
                      <Card  style={{ background: '#fff2f0', border: '1px solid #ffccc7' }}>
                        <div style={{ textAlign: 'center' }}>
                          ...
                          <Progress
                            percent={(server.hardware.cpu.temperature_celsius.package / 100) * 100}
                            format={() => `${server.hardware.cpu.temperature_celsius.package.toFixed(1)}°C`}
                            strokeColor="#ff4d4f"
      ...
      <Col xs={24}>
        <GrafanaPanel
          ...
          subtitle="SSD/NVMe/HDD performance with TBW tracking and thermal monitoring"
          height={600}
        >
          <Row gutter={[16, 16]}>
                ...
                <Table
                  dataSource={servers.flatMap(server =>
                    server.hardware.storage.devices.map((device, idx) => ({
                      ...
                      used: `${device.used_gb}GB (${device.usage_percent.toFixed(1)}%)`,
                      temperature: `${device.temperature_celsius.toFixed(1)}°C`,
                      fluctuation: `±${device.temperature_fluctuation_5min.toFixed(1)}°C`,
                      tbw: device.tbw_total ?
                        `${device.tbw_written.toFixed(2)} / ${device.tbw_total}TB (${device.tbw_percentage}%)` :
                        'N/A',
                      ...
                      statusTag: (
                        <Tag color={
                          device.smart_status === 'healthy' ? 'green' :
                          device.smart_status === 'warning' ? 'orange' : 'red'
                        }>
                  ...
                  columns={[
                    ...
                    { title: 'Model', dataIndex: 'model', width: 150 },
                    {
                      ...
                      width: 80,
                      render: (type: string) => (
                        <Tag color={type === 'NVME' ? 'blue' : type === 'SSD' ? 'green' : 'orange'}>
                          {type}
                        </Tag>
                      )
                    },
                    ...
                    { title: 'Usage', dataIndex: 'used', width: 120 },
                    {
                      ...
                      render: (temp: string, record: any) => {
                        ...
                        const color = tempVal > 70 ? '#ff4d4f' : tempVal > 60 ? '#faad14' : '#52c41a';
                        return <span style={{ color }}>{temp}</span>;
                      }
                    },
                    { title: 'Fluctuation', dataIndex: 'fluctuation', width: 120 },
            ...
            <Col span={24}>
              <Space direction="vertical" style={{ width: '100%' }}>
                {servers.slice(0, 3).map((server, idx) => (
                  <Card key={server.hostname} size="small" title={server.hostname}>
                    <Row gutter={16}>
                      <Col span={12}>
                        <Space direction="vertical">
                          <div>
                            ...
                            <div>
                              {server.hardware.power.psu_status.map((status, i) => (
                                <Tag
                                  key={i}
                                  color={status === 'healthy' ? 'green' : status === 'warning' ? 'orange' : 'red'}
                                  style={{ margin: '2px' }}
                                >
                          ...
                          <div>
                            ...
                            <div style={{ fontSize: '16px', fontWeight: 'bold' }}>
                              ...
                              {server.hardware.power.power_consumption_watts.toFixed(1)}W
                              <Text type="secondary" style={{ fontSize: '12px', marginLeft: '8px' }}>
                                (peak: {server.hardware.power.power_consumption_peak_watts.toFixed(1)}W)
                              </Text>
                            </div>
                          </div>
                          <div>
                            ...
                            <Progress
                              percent={server.hardware.power.power_efficiency_percent}
                              size="small"
                              strokeColor={server.hardware.power.power_efficiency_percent > 85 ? '#52c41a' : '#faad14'}
                            />
                          </div>
                        </Space>
                      </Col>
                      <Col span={12}>
                        <Space direction="vertical">
                          ...
                          <div style={{ fontSize: '12px' }}>
                            <div>3.3V: {server.hardware.power.voltage_levels['3.3v'].toFixed(2)}V</div>
                            <div>5V: {server.hardware.power.voltage_levels['5v'].toFixed(2)}V</div>
                            <div>12V: {server.hardware.power.voltage_levels['12v'].toFixed(2)}V</div>
                          </div>
                          <Divider style={{ margin: '8px 0' }} />
                          <Text strong>Fluctuation (5s peak)</Text>
                          <div style={{ fontSize: '12px' }}>
                            <div>3.3V: ±{server.hardware.power.voltage_levels['3.3v_fluctuation'].toFixed(2)}V</div>
                            <div>5V: ±{server.hardware.power.voltage_levels['5v_fluctuation'].toFixed(2)}V</div>
                            <div>12V: ±{server.hardware.power.voltage_levels['12v_fluctuation'].toFixed(2)}V</div>
                          </div>
                        </Space>
                      </Col>
                    </Row>
                  ...
                  <Card key={server.hostname} size="small" title={server.hostname}>
                    <Row gutter={16}>
                      <Col span={12}>
                        ...
                        <Divider style={{ margin: '8px 0' }} />
                        <Space direction="vertical" style={{ width: '100%' }}>
                          {server.hardware.power.fans.cpu_fans?.map((fan, i) => (
                            <div key={i}>
                              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                <Text type="secondary">{fan.label}</Text>
                                <Tag color={
                                  fan.status === 'normal' ? 'green' :
                                  fan.status === 'warning' ? 'orange' : 'red'
                                }>
                              ...
                              <Progress
                                ...
                                strokeColor={fan.speed_percent > 80 ? '#ff4d4f' : fan.speed_percent > 60 ? '#faad14' : '#52c41a'}
                              />
                              <Text type="secondary" style={{ fontSize: '11px' }}>
                                Target: {fan.target_rpm} RPM | Fluctuation: ±{fan.speed_fluctuation_5min.toFixed(0)} RPM
                              </Text>
                            </div>
                          ))}
                        </Space>
                        ...
                        <Space direction="vertical" style={{ width: '100%' }}>
                          {server.environment?.cooling.fans.map((fan, i) => (
                            <div key={i}>
                              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                <Text type="secondary">{fan.label}</Text>
                                <Tag color={fan.status === 'normal' ? 'green' : 'orange'}>
                                  {fan.status.toUpperCase()}
                                </Tag>
                              </div>
                              <Progress
                              ...
                              />
                              <Text type="secondary" style={{ fontSize: '11px' }}>
                                Efficiency: {fan.efficiency_percent}% | Temp Correlated: {fan.temperature_correlated ? 'Yes' : 'No'}
                              </Text>
                            </div>
                          ))}
                        </Space>
      ...
      <Col xs={24}>
        <GrafanaPanel
          ...
          subtitle="Peak performance capture for workload optimization"
          height={300}
        >
          <Row gutter={[16, 16]}>
            {servers.slice(0, 4).map(server => (
              <Col xs={24} sm={12} lg={6} key={server.hostname}>
                <Card
                  ...
                  size="small"
                  extra={
                    <Tag color={server.status === 'healthy' ? 'green' : server.status === 'warning' ? 'orange' : 'red'}>
                ...
                >
                  <Row gutter={[8, 8]}>
                    ...
                    <Col span={12}>
                      <Statistic
                        title="Temp Peak"
                        value={server.hardware.cpu.temperature_celsius.package.toFixed(1)}
                        suffix="°C"
                    ...
                    <Col span={24}>
                      ...
                      <div style={{ fontSize: '12px', marginTop: '4px' }}>
                        <div>Read: {server.hardware.storage.devices[0]?.read_iops_peak.toLocaleString() || 0}</div>
                        <div>Write: {server.hardware.storage.devices[0]?.write_iops_peak.toLocaleString() || 0}</div>
                      </div>
                    </Col>
                    <Col span={24}>
                      <Text type="secondary">Power Peak</Text>
                      <div style={{ fontSize: '14px', fontWeight: 'bold', color: '#52c41a' }}>
                        {server.hardware.power.power_consumption_peak_watts.toFixed(1)}W
                      </div>
                    </Col>
                  </Row>
                </Card>
