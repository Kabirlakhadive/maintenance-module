...
export const useMockData = () => {
  ...
  const healthScore = totalServers > 0 ? Math.round((healthyServers / totalServers) * 100) : 0;

  const avgCPU = servers.length > 0
    ? Math.round(servers.reduce((sum, s) => sum + s.hardware.cpu.utilization_percent, 0) / servers.length)
    : 0;

  const avgMemory = servers.length > 0
    ? Math.round(servers.reduce((sum, s) => sum + s.hardware.memory.usage_percent, 0) / servers.length)
    : 0;
